...
import numpy
from numpy import pi, abs, sqrt, cos, sin, arctan, tan
import math
...
class MarkleyKESolver:
  ...
  def _alpha(self, e, M):
    ...
    return ( 3.*pi**2 + 1.6*pi*(pi-M)/(1.+e) )/(pi**2 - 6.)
  ...
  def _w(self, r, q):
    ...
    if r < 0:
        r = -r 
    return (r + math.sqrt(q**3 + r**2))**(2./3.)
    #return (abs(r) + math.sqrt(q**3 + r**2))**(2./3.)
    #return (abs(r) + (q**3 + r**2)**(1./2.))**(2./3.)
